...
class SMGSticky(CustomLogger):
    async def async_pre_call_hook(self, user_api_key_dict, cache, data, call_type):
        ...
        # If no stable identity exists, fall back to per-request call id.
        # That avoids routing all anonymous traffic to one DP rank.
        stable_part = first(session, user, data.get("litellm_call_id"))
        raw_key = f"{tenant}:{agent}:{workspace}:{stable_part}"
