# Sticky routing probes

Сценарии перенесены из `sticky-sessions/`. Добавлены SMG 1.10.1, Collector и
локальный Jaeger; параметры sticky policy сохранены. Для полного trace внешние
workers и LiteLLM из `one-4090` также должны включать W3C/OTLP по
[общей инструкции](../telemetry/README.md). Здесь есть внешние зависимости.
Эти сценарии не проверялись повторно на GPU после переноса.
...
```bash
cd nyashkimyashki/sglang/qwen3-8b-sticky/one-4090
docker compose up -d
```
...
cd nyashkimyashki/sglang/qwen3-8b-sticky/two-4090-dpa
