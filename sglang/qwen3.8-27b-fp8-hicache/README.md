...
Состав: **sglang + SMG + Collector+ Jaeger**.
одна GPU 48 GiB, FP8 KV, SSD HiCache, MTP, gRPC/KV events. Параметры модели и cache budgets сохранены при переносе.
...
Клиент обращается к SMG на `http://<host>:30000/v1`. Этот локальный gRPC recipe
сохраняет прежнюю auth-границу перед SMG. Для LAN задайте `API_BIND=<LAN IP>`;
по умолчанию API слушает localhost.
...
```bash
cd sglang/qwen3.8-27b-fp8-hicache
cp .env.example .env
```
