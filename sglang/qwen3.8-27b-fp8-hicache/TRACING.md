...
- Локальный Jaeger 2.20.0: локальный UI, временное хранение до 2000 traces
...
Tracing, Collector и Jaeger входят в `docker-compose.yaml` по умолчанию:
...
Для экспериментов добавьте `RESTART_POLICY=no` в локальный `.env`. Для внешнего
backend используйте `-f docker-compose.yaml -f compose.external.yaml` и задайте
`OTLP_UPSTREAM_ENDPOINT`; Jaeger в этом режиме не запускается. Старые overlays
`compose.tracing.yaml` и `compose.trace-ui.yaml` оставлены для совместимости.
Общая политика фильтрации находится в `../../misc/telemetry/otel-collector.yaml`.
